...
public class oldCreerTournoiController {
    ...
    public oldCreerTournoiController() throws SQLException {
    }
    ...
    public void choisir(ActionEvent actionEvent) throws SQLException, IOException {
        if (id_t == -1) {
            ...
            if (isValidTournoiId(enteredValue)) {
                ...
                showAlert("ID Invalide", "La valeur saisie n'est pas un ID valide d'un tournoi.");
            }
        }else{
            if (updating==1){
                if (id.getText().isEmpty() || jeu.getValue() == null || type.getText().isEmpty() || date.getValue() == null ) {
    ...
    public void supprimer (ActionEvent actionEvent) throws SQLException{
        ...
        tr.supprimer(id_t);
        showAlert("Succès", "Le tournoi a été supprimé");
